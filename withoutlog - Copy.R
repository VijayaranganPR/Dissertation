...
plot(log(1+loan_data_clean_non$total_esg_score), log(1+loan_data_clean_non$allindrawn))
plot(loan_data_clean_non$total_esg_score, loan_data_clean_non$allindrawn)
...
loan_data_non <- loan_data_clean_non %>%
  ...
  mutate(across(c(allindrawn, total_assets, facilityamt, num_lenders), ~log(1 + .x), .names = "log_{col}"))
...
data_analysis <- loan_data_non %>%
  select(total_esg_score, spread, log_spread, 
         numcov1,
         size, log_size, 
...
datasummary(total_esg_score + spread + log_spread+ 
            numcov1+
            size+ log_size+ 
            facilityamt+ log_facilityamt+
            num_lenders+ log_num_lenders+ 
...
est2 <- loan_data_non %>%
  feols(log_spread ~ total_esg_score + profitability_w + leverage_w + log_size+ industry)
...
est1 <- loan_data_non %>%
  feols(numcov1 ~ total_esg_score)
est2 <- loan_data_non %>%
  feols(numcov1 ~ total_esg_score + log_facilityamt + log_num_lenders + profitability_w + leverage_w + log_size+ industry)
